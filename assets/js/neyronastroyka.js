            ...
            <div class="container contact-wrapper">
                <div class="contact-text" id="sofia-contact">
                    ...
                    <p>
                        ...
                        напишите моей ассистентке Софии в Telegram, VK, или WhatsApp.
                        Она с теплотой откликнется, поддержит и мягко направит,
...
function buildPackLinks(neyronastroyki, allNeyronastroyki) {
  if (!neyronastroyki || !neyronastroyki.length) return "";
  const links = neyronastroyki
    .map((neyronastroykaId) => {
      const neyronastroyka = allNeyronastroyki.find(
        (p) => p.slug === neyronastroykaId,
      );
      if (!neyronastroyka) return "";
      return `<a href="${neyronastroyka.slug}.html" aria-label="Перейти к нейронастройке ${neyronastroyka.title}">${neyronastroyka.title}</a>`;
    })
    .filter(Boolean)
    .join(", ");
  if (!links) return "";
...
/* ── BUILD PAGE ──────────────────────────────────────────── */
function buildPage(neyronastroyka, allNeyronastroyki) {
  console.log("Building page for:", neyronastroyka.title);
  const price = neyronastroyka.price
    ? `${neyronastroyka.price.toLocaleString("ru-RU")} ₽`
    : "Бесплатно";
  ...
  const problemParas = (neyronastroyka.problemSectionDescription || [])
  ...
  const howItLooksSection =
    neyronastroyka.howItLooksBooletPoints &&
    neyronastroyka.howItLooksBooletPoints.length
      ? `<section class="med-single-section">
            <div class="med-how-inner">
              <h2 class="section-title">${neyronastroyka.howItLooksTitle}</h2>
              <ul class="med-how-list med-single-how-list">
                ${buildBulletList(neyronastroyka.howItLooksBooletPoints)}
              </ul>
            </div>
          </section>`
      : "";
  ...
  const effectsSection =
    neyronastroyka.effectsBooletPoints &&
    neyronastroyka.effectsBooletPoints.length
      ? `<section class="med-single-section">
            <div class="med-how-inner">
              <h2 class="section-title">${neyronastroyka.effectsTitle}</h2>
              <ul class="med-how-list med-single-how-list">
                ${buildBulletList(neyronastroyka.effectsBooletPoints)}
              </ul>
            </div>
          </section>`
      : "";
  ...
  const howItHelpsParas = (neyronastroyka.howItHelpsBooletPoints || [])
  ...
  const html = `
          ...
          <div class="container pack-single-hero-inner">
            <div class="container med-hero-grid">
              <div class="med-hero-content med-single-hero-content">
                <p class="med-hero-badge">${neyronastroyka.meditationHeroBadge}</p>
                <h1 class="section-title">${neyronastroyka.title}</h1>
                <p class="med-hero-subtitle med-single-hero-subtitle">${neyronastroyka.shortDescriptionHero}</p>
                <div class="med-packs-card-action">
                  ...
                  <a href="#how-to-buy" class="btn-primary med-packs-card-btn"
                     aria-label="Получить медитацию ${neyronastroyka.title.replace(/<br\s*\/?>/gi, " ")}">
                    Получить полную версию
                  </a>
                </div>
                ${buildPackLinks(neyronastroyka.includedInPacks, allNeyronastroyki)}
              </div>
              <figure class="med-hero-photo-wrap med-single-hero-photo-wrap">
                <img src="${neyronastroyka.image}" class="med-hero-photo"
                     alt="${neyronastroyka.title.replace(/<br\s*\/?>/gi, " ")} — медитация Сурии">
              </figure>
            </div>
          </div>
        </section>
        ...
        <section class="med-single-section">
          <div class="med-single-wrap">
            <div class="pack-single-problem-content">
              <h2 class="section-title">${neyronastroyka.problemSectionTitle}:</h2>
              <h2 class="section-title med-title-accent">${neyronastroyka.problemSectionTitleAccent}</h2>
              ${problemParas}
            </div>
          </div>
        </section>
        ...
        <section class="med-how-final">
          <div class="med-how-final-wrapper">
            <h2 class="section-title">${neyronastroyka.howItHelpsTitle}</h2>
            ${howItHelpsParas}
          </div>
        </section>
...
fetch(JSON_PATH)
  ...
  .then((data) => {
    const neyronastroyka = (data.neyronastroyki || data.meditations || []).find(
      (m) => m.slug === slug,
    );
    ...
    buildPage(neyronastroyka, data.packs || []);
  })
  .catch((err) => {
