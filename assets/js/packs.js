    ...
    <section id="contact" class="alt contact">
      <div class="container contact-wrapper">
        ...
        function buildHowToListen(pack, allMeditations) {
            ...
            const items = pack.howToListen.map(step => {
                ...
                let enriched = step;
                allMeditations.forEach(m => {
                    const plainTitle = m.title.replace(/<br\s*\/?>/gi, ' ').replace(/\s+/g, ' ').trim();
      ...
      <section class="med-how">
        ...
        function buildMeditationCards(pack, allMeditations) {
            ...
            const cards = pack.meditations.map((slugOrRef, stepIndex) => {
                ...
                const stepDesc = stepData ? stepData.discription : (med ? med.shortDescriptionCommon : '');
                const stepTitle = stepData ? stepData.title : (med ? med.title.replace(/<br\s*\/?>/gi, ' ') :
                    slug);
      ...
      <section class="med-single-section">
        ...
        function buildHowItLooks(pack) {
            const points = (pack.howItLooksBooletPoints || []).filter(p => p && p.trim());
      ...
      <section class="med-single-section">
        ...
        function buildPage(pack, allMeditations) {
            ...
            document.querySelector('meta[name="description"]').content = pack.shortDescriptionCommon;
            document.querySelector('link[rel="canonical"]').href = pageUrl;
            document.querySelector('meta[property="og:title"]').content = seoTitle;
            document.querySelector('meta[property="og:description"]').content = pack.shortDescriptionCommon;
            ...
            document.querySelector('meta[name="twitter:description"]').content = pack.shortDescriptionCommon;
